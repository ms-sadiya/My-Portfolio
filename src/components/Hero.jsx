...
const Hero = ({ darkMode, scrollToSection }) => {
  return (
    <section id="home" className="min-h-screen flex items-center justify-center pt-20 pb-12 px-4 overflow-hidden">
      <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        ...
        >
          ...
          >
            I'm <span className={`font-bold ${darkMode ? 'text-white' : 'text-slate-900'}`}>Ansari Sadiya</span>, a MERN Stack Developer building high-performance, accessible applications.
          </motion.p>
          ...
          >
            ...
            <div className="flex items-center gap-4 ml-2">
              ...
              <a href="https://www.linkedin.com/in/ansari-sadiya003" target="_blank" rel="noreferrer" className={`p-4 ${darkMode ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'} rounded-2xl hover:text-indigo-500 hover:scale-110 transition-all shadow-sm`}>
                <Linkedin size={24} />
              </a>
            </div>
          </motion.div>
        ...
        >
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] pointer-events-none">
            <div className={`absolute top-0 right-10 w-72 h-72 rounded-full blur-[100px] opacity-40 animate-pulse`} 
                 style={{ background: 'linear-gradient(135deg,#6366f1,#a855f7)' }} />
            <div className={`absolute bottom-0 left-10 w-64 h-64 rounded-full blur-[100px] opacity-30 animate-bounce-slow`} 
                 style={{ background: 'linear-gradient(135deg,#ec4899,#f97316)' }} />
          </div>

          <div className={`relative z-10 p-4 rounded-[3.5rem] transition-colors duration-500 ${darkMode ? 'bg-slate-900/40' : 'bg-white/60'} backdrop-blur-xl border ${darkMode ? 'border-white/10' : 'border-slate-200'} shadow-[0_32px_64px_-12px_rgba(0,0,0,0.1)] group`}>
            ...
            <div className="relative">
              ...
              <div className={`rounded-[2.8rem] p-2 ${darkMode ? 'bg-linear-to-tr from-indigo-500 via-purple-500 to-pink-500' : 'bg-linear-to-tr from-indigo-200 via-purple-200 to-pink-200'}`}>
                <div className={`w-72 h-72 md:w-80 md:h-80 rounded-[2.5rem] overflow-hidden shadow-inner relative transition-transform duration-500 group-hover:scale-[1.02]`}>
                  <img 
                    src={profilePic} 
                    alt="Ansari Sadiya" 
                    className="object-cover w-full h-full grayscale-20 group-hover:grayscale-0 transition-all duration-700"
                    onError={(e) => { e.target.src = "https://ui-avatars.com/api/?name=Ansari+Sadiya&background=6366f1&color=fff&size=512"; }}
                  />
            ...
            <div className={`mt-6 text-center pb-4`}>
              ...
              <p className={`text-sm font-medium mt-1 flex items-center justify-center gap-2 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                <Layers size={14} className="text-indigo-500" /> Mern Stack Developer
              </p>
            </div>
          </div>
        </motion.div>
